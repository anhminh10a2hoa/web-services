//This is soap/service/ObjectExchangeService.java file
...
public class ObjectExchangeService {
	...
	public OMElement uploadImageOMElement(OMElement element) {
		...
		OMFactory omFactory = OMAbstractFactory.getOMFactory();
		OMNamespace omNameSpace = omFactory.createOMNamespace(this.nameSpaceURI, "su");
		OMElement methodElement = omFactory.createOMElement("uploadOMElementResponse", omNameSpace);
		OMNode dataOMNode = ((OMElement) element.getChildrenWithLocalName("file_content").next()).getFirstOMChild();
		FileOutputStream fileOutputStream = null;
		DataHandler dataHandler = null;
		...
		try {
			...
			confirmation.append(e.getMessage());
			methodElement.setText(confirmation.toString());
		} finally {
			try {
				fileOutputStream.close();
			} catch (IOException e) {
		...
		confirmation.append(file.getAbsolutePath() + " uploaded? " + file.exists() + " length:" + file.length());
		// methodElement.addChild(omFactory.createOMText(methodElement,
		// confirmation));
		methodElement.setText(confirmation.toString());
		return methodElement;
	}
